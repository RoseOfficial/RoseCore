...
function RoseWHM.Cast()
    ...
    if RoseCore.Settings.Active then
        ...
        -- Show a debug of current target's cast
        if table.valid(target) then
            local cast = nil
            if target.castinginfo then
                cast = target.castinginfo.channelingid
                RoseWHM.DebugPrint("Current target is casting: " .. cast)
            end
        end
            ...
            if WhmHotbarSettings.Heal.bool then
                ...
                for k, v in pairs(RoseWHM.ActionQueue) do
                    ...
                    if v.shouldBeUsed then
                        ...
                        if Player.castinginfo.castingid == spell.ID then -- Get last cast id
                            RoseWHM.DebugPrint("Removed " .. v.actionName .. " from the queue ("..#RoseWHM.ActionQueue - 1 .." left)")
                            table.remove(RoseWHM.ActionQueue, k)
                        end
                    else
                        local action = ActionList:Get(1, spell.ID)
                        local cooldown = action.cdmax - action.cd
                        if action.cd ~= nil and action.cd ~= 0 then
                            if cooldown > 5 then
                                RoseWHM.DebugPrint("Removed " .. v.actionName .. " from the queue due to being under CD ("..#RoseWHM.ActionQueue - 1 .." left)")
                                table.remove(RoseWHM.ActionQueue, k)
                            end
                        end
                    end
...
function RoseWHM.HandleAOEProtection(level, khpcount, tanks)
    ...
    -- This part is to avoid casting a skill right before the AOE goes off as it would result of the spell being casted AFTER the AOE, so useless.
    -- This part can probably be improved by just getting the Unavoidable and not the first one on the list only but I don't know how to do that yet.
