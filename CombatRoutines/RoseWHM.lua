...
-- Utility function: table.contains
...
function RoseWHM.GetRoleByJob(job)
    local tankJobs = {1, 3, 19, 21, 32, 37} -- Add or modify the tank jobs as needed (using numerical job codes)
    if table.contains(tankJobs, job) then
        return "tank"
    else
        -- Add other role checks for DPS and Healer jobs if necessary
    end
    return nil
end
...
function RoseWHM.ShouldApplyDoT(enemies)
    -- d("RoseWHM.ShouldApplyDoT called")
    ...
    for _, enemy in pairs(enemies) do
        ...
        for _, buff in pairs(enemy.buffs) do
            if (buff.id == Aero or buff.id == AeroII or buff.id == Dia) and tonumber(buff.ownerid) == tonumber(playerId) then
                ...
                if buff.duration <= 3 then
                    -- d("DoT duration is less than or equal to 3")
                    return true, enemy
                end
            end
        end
        ...
        if not hasDoTBuff then
            -- d("Enemy does not have a DoT buff")
            return true, enemy
        end
    end
    return false, nil
...
function RoseWHM.ActionIsReady(id)
    if (MIsLoading() or not ActionList:IsReady()) then
        -- d("Action list is not ready or game is loading.")
        return false
    end
    ...
    if (table.valid(action)) then
        ...
        if not isOffCooldown then
            -- d(string.format("Spell with ID: %d is on cooldown", id))
        end
        if not hasEnoughResources then
            -- d(string.format("Not enough resources to cast spell with ID: %d", id))
        end
...
function RoseWHM.CastSpellIfReady(spellId, targetId)
    if RoseWHM.ActionIsReady(spellId) then
        ...
        -- d(string.format("Casting spell with ID: %d on target with ID: %d", spellId, targetId))
        spell:Cast(targetId)
        return true
    else
        -- d(string.format("Spell with ID: %d is not ready", spellId))
    end
    return false
end
...
function RoseWHM.GetBestRevive(party, role)
    ...
    if (table.valid(el)) then
        ...
        RoseWHM.Debug("No valid revive targets found")
    end
    ...
    if table.valid(PartyMembersDead) and Player.level >= 12 and Player.mp.percent >= 24 then
        ...
        if table.valid(raiseTarget) and MissingBuffs(raiseTarget, "148+1140") then
            ...
            if Player.level >= 18 and RoseWHM.CastSpellIfReady(swiftcastId, Player.id) then
                RoseWHM.Debug("raise", "Using Swiftcast")
            ...
            elseif RoseWHM.CastSpellIfReady(raiseId, raiseTarget.id) then
                if TimeSince(reztime) > 2000 then
                    --SendTextCommand("/p Rezzing <t>")
                    reztime = Now()
                    RoseWHM.Debug("raise", "Casting Raise")
                    return RoseWHM.CastSpellIfReady(raiseId, raiseTarget.id)
                end
            else
                RoseWHM.Debug("raise", "Raise spell is not ready")
            end
        else
            RoseWHM.Debug("raise", "No valid raise target or target has Raise immunity")
        end
    else
        RoseWHM.Debug("raise", "No dead party members or insufficient level/MP")
    end
end
    ...
    if target then
        ...
        while not swiftcastApplied and os.clock() - startTime < 3 do
            ...
            if swiftcastBuff then
                RoseWHM.Debug("Swiftcast applied")
        ...
        if swiftcastApplied then
            RoseWHM.Debug("Casting Raise with Swiftcast")
            RoseWHM.CastSpellIfReady(raiseId, target.id)
        else
            RoseWHM.Debug("Swiftcast not applied, unable to cast Raise with Swiftcast")
        end
    else
        RoseWHM.Debug("No valid target for Swiftcast Raise")
    end
end
...
function RoseWHM.GetHealableTargets(range, hp, whitelist, npc)
    ...
    -- Add the player as a healing target if their HP is within the threshold
...
function RoseWHM.Targeting()
    ...
    local partyMembers = RoseWHM.GetPartyMembers() -- Call the GetPartyMembers function instead
...
function RoseWHM.GetPartyMembers()
    ...
    -- Add the Player to the partyMembers table
    partyMembers[Player.id] = Player

    -- Add Trusts to the partyMembers table
    ...
    RoseWHM.Debug("party", "Found " .. tostring(table.size(partyMembers)) .. " party members")
    return partyMembers
end
...
function RoseWHM.UseSprint()
    ...
    if sprintAction then
        ...
        if sprintRecastTime == 0 and sprintAction:IsReady() and playerIsMoving then
            ...
            -- d("Sprint is on cooldown or player is not moving")
        end
    else
        -- d("Sprint action not found")
    end
    ...
    if lucidDreamingAction then
        ...
        if Player.mp.percent < 80 then
            if lucidDreamingRecastTime == 0 and lucidDreamingAction:IsReady() then
                -- d("Casting Lucid Dreaming")
                lucidDreamingAction:Cast(Player.id)
                return true
            else
                -- d("Lucid Dreaming is on cooldown")
            end
        else
            -- d("MP is above 80%")
        end
    else
        -- d("Lucid Dreaming action not found")
    end
        ...
        if entity then
            ...
            for _, buff in ipairs(buffs) do
                if buff.id == buffID then
                    if buffID == 418 then
                        RoseWHM.Debug("cast", "Player has Transcendent buff, stopping all actions")
                    end
                    return true
                end
            end
...
function RoseWHM.SingleTargetDamage()
    ...
    if currentTarget and currentTarget.incombat then
        -- Check if Presence of Mind is off cooldown and cast it
...
function RoseWHM.IsCasting()
    ...
    if castingInfo and castingInfo.iscasting then
        return true
    else
        return false
    end
...
function RoseWHM.FollowTank()
    local tank = RoseWHM.GetTank()

    -- Check if the player is casting before initiating any movement
    if RoseWHM.IsCasting() then
        RoseWHM.Debug("follow", "Player is casting, not moving")
        return
    end

    if tank and tank.pos then
        local tankDistance = math.distance3d(Player.pos, tank.pos)
        local enemies = RoseWHM.elist
        local clusteredEnemy, isClustered

        if not RoseWHM.ClusteredEnemiesAlive(currentClusteredEnemies) then
            clusteredEnemy, isClustered, currentClusteredEnemies = RoseWHM.FindClusteredEnemy(enemies)
        end

        if table.valid(enemies) and isClustered then
            local newPos = RoseWHM.GetNewPosition(clusteredEnemy.pos, 5)
            if newPos then
                local currentDistance = math.distance3d(Player.pos, newPos)
                if currentDistance > 5 then
                    RoseWHM.Debug("follow","Moving to clustered enemy position (More than 1 enemy in range)")
                    Player:MoveTo(newPos.x, newPos.y, newPos.z)
                else
                    RoseWHM.Debug("follow","Already within the desired distance of the clustered enemies")
                    Player:Stop()
                end
            else
                RoseWHM.Debug("follow","Cannot find a valid position near the clustered enemy")
            end
        end

        if table.valid(enemies) then
            local singleEnemy = RoseWHM.GetClosestEnemy(enemies)
            local newPos = RoseWHM.GetNewPosition(singleEnemy.pos, 10)
            if newPos then
                local currentDistance = math.distance3d(Player.pos, newPos)
                if currentDistance > 10 then
                    if not Player.isengaged or Player.targetid ~= singleEnemy.id then
                        RoseWHM.Debug("follow","Moving to single enemy position (Only 1 enemy in range)")
                        Player:MoveTo(newPos.x, newPos.y, newPos.z)
                    else
                        RoseWHM.Debug("follow","Already engaged with the single enemy, not moving")
                    end
                elseif currentDistance > 15 then
                    RoseWHM.Debug("follow","Enemy moved more than 15 units away, following the enemy again")
                    Player:MoveTo(newPos.x, newPos.y, newPos.z)
                else
                    RoseWHM.Debug("follow","Already within the desired distance of the single enemy")
                    Player:Stop()
                end
            else
                RoseWHM.Debug("follow","Cannot find a valid position near the single enemy")
            end
        end

        if not table.valid(enemies) or math.distance3d(Player.pos, RoseWHM.GetClosestEnemy(enemies).pos) > 30 then
            if tankDistance > 15 then
                local newPos = RoseWHM.GetNewPosition(tank.pos, 15)
                if newPos then
                    local currentDistance = math.distance3d(Player.pos, newPos)
                    if currentDistance > 15 then
                        RoseWHM.Debug("follow","Moving closer to the tank (No enemies in range)")
                        Player:MoveTo(newPos.x, newPos.y, newPos.z)
                    else
                        RoseWHM.Debug("follow","Already within the desired distance from the tank")
                        Player:Stop()
                    end
                else
                    RoseWHM.Debug("follow","Cannot find a valid position near the tank")
                end
            else
                RoseWHM.Debug("follow","Already within the desired distance from the tank")
                Player:Stop()
            ...
            RoseWHM.Debug("follow", "Already within the desired distance from the single enemy, not following the tank")
        end
    else
        -- RoseWHM.Debug("follow","No tank found")
    end
end

function RoseWHM.ClusteredEnemiesAlive(clusteredEnemies)
    if not table.valid(clusteredEnemies) then
        return false
    end

    for _, enemy in ipairs(clusteredEnemies) do
...
function RoseWHM.GetTank()
    local partyMembers = RoseWHM.GetPartyMembers()
    for _, member in pairs(partyMembers) do
        local role = RoseWHM.GetRoleByJob(member.job)
        -- RoseWHM.Debug("get_tank", "Member: " .. member.name .. " | Job: " .. member.job .. " | Role: " .. tostring(role))
        if role == "tank" then
            -- RoseWHM.Debug("get_tank", "Tank found: " .. member.name)
            return member
        end
    end
    -- RoseWHM.Debug("get_tank", "No tank found")
    return nil
end

function RoseWHM.FindClusteredEnemy(enemies)
    if not table.valid(enemies) then
        return nil, false
    end
    local threshold = 5 -- You can adjust this value based on your preference
    local maxCount = 0
    local clusteredEnemy = nil

    for _, enemy in pairs(enemies) do
        local count = 0
        for _, otherEnemy in pairs(enemies) do
            if enemy.id ~= otherEnemy.id and math.distance3d(enemy.pos, otherEnemy.pos) < threshold then
                count = count + 1
            end
        end
        ...
        if count > maxCount then
            maxCount = count
            clusteredEnemy = enemy
        end
    end

    return clusteredEnemy, maxCount > 0
end

function RoseWHM.GetClosestEnemy(enemies)
    local minDistance = math.huge
    local closestEnemy = nil

    for _, enemy in pairs(enemies) do
        local distance = math.distance3d(Player.pos, enemy.pos)
...
function RoseWHM.GetNewPosition(targetPos, desiredDistance, maxAttempts)
    local currentDistance = math.distance3d(Player.pos, targetPos)

    -- Check if the player is already within the desired distance range
    ...
    local angle = math.atan2(targetPos.z - playerPos.z, targetPos.x - playerPos.x)

    for attempt = 1, maxAttempts do
        local newPos = {
            x = targetPos.x + desiredDistance * math.cos(angle),
            y = targetPos.y,
            z = targetPos.z + desiredDistance * math.sin(angle)
        }

        if NavigationManager:IsReachable(newPos) then
            return newPos
        else
            -- If the new position is not reachable, try moving in smaller increments
            local step = desiredDistance / (attempt + 1)
            newPos = {
                x = targetPos.x + step * math.cos(angle),
                y = targetPos.y,
                z = targetPos.z + step * math.sin(angle)
            }
...
--[[RoseWHM.DamageData = {}
RoseWHM.PreviousHealth = {}

function RoseWHM.UpdateDamageData(partyMembers)
    for _, member in pairs(partyMembers) do
        local entityId = member.id
        local currentHealth = member.hp.current
        local currentTime = os.time()

        if not RoseWHM.PreviousHealth[entityId] then
            RoseWHM.PreviousHealth[entityId] = currentHealth
        end

        local healthDifference = RoseWHM.PreviousHealth[entityId] - currentHealth

        -- Check if healthDifference is positive before updating DamageData and PreviousHealth
        if healthDifference > 0 then
            if not RoseWHM.DamageData[entityId] then
                RoseWHM.DamageData[entityId] = {}
            end
            ...
            table.insert(RoseWHM.DamageData[entityId], damageEvent)

            -- Save data to a file
            local damageDataFile = io.open("RoseWHM_DamageData.txt", "a")
            damageDataFile:write(string.format("EntityID: %d, Time: %d, Health: %d, DamageTaken: %d\n", entityId, currentTime, currentHealth, healthDifference))
            damageDataFile:close()

            -- Update PreviousHealth only if damage is taken
            RoseWHM.PreviousHealth[entityId] = currentHealth
        end
    end
end

function RoseWHM.DisplayDamageData()
    -- d("Damage Data:")
    local partyMembers = RoseWHM.PartyMembers()
    for entityId, damageEvents in pairs(RoseWHM.DamageData) do
        local entity = partyMembers[entityId]
        if entity then
            local totalDamageTaken = 0
            for _, event in ipairs(damageEvents) do
                totalDamageTaken = totalDamageTaken + event.damageTaken
            end

            -- d(string.format("Entity ID: %d, Name: %s, Damage Taken: %d", entityId, entity.name, totalDamageTaken))
        end
    end
end]]
...
function RoseWHM.OnClick(mouse,shiftState,controlState,altState,entity)
    -- d(string.format("[RoseWHM] OnClick function called with parameters: mouse=%d,shiftState=%s, controlState=%s, altState=%s, entity=%s", mouse, tostring(shiftState), tostring(controlState), tostring(altState), tostring(entity)))
end
